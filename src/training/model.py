...
from src.training.ops import *
from src.training.utils import *

#卷積之前 預處裡outputsize和stride
...
class DCGAN(object):
  ...
  def build_model(self, old_model=False):
    ...
    if self.can:
      ...
      #true_label、false_label假設 防止 值的range太小
      ...
      self.g_loss_fake = -tf.reduce_mean(self.D_)
      ...
      with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
          self.d_optim = tf.train.AdamOptimizer(0.0002, beta1=0.5) \
              .minimize(self.d_loss, var_list=self.d_vars)
          self.g_optim = tf.train.AdamOptimizer(0.0002, beta1=0.5) \
              .minimize(self.g_loss, var_list=self.g_vars)

      #weight clipping WGAN優化方式
      self.clip_D = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in self.d_vars]
      ########

      ###Summery### D、G的tensorboard設定
      ...
      self.g_loss_fake_sum = scalar_summary("g_loss_fake", self.g_loss_fake)
  ...
  def train(self, config):
    ...
    if self.can:
      self.d_sum = merge_summary(
        [self.z_sum, self.d_sum, self.d_loss_real_sum, self.d_loss_sum, self.d_loss_class_real_sum,self.g_loss_fake_sum, self.g_loss_class_fake_sum])
    ...
    if self.y_dim:
      sample_files = self.data[0:self.sample_num] #取data 數量
      sample = [
          get_image(sample_file, #data預處理
      ...
      if (self.grayscale):#如果是灰階
        sample_inputs = np.array(sample).astype(np.float32)[:, :, :, None] #把input 換成矩陣 並改成float型式
      else:
        sample_inputs = np.array(sample).astype(np.float32) 
      sample_labels = self.get_y(sample_files) #取sample_files的label

    else:
      sample_files = self.data[0:self.sample_num]  # 取幾個data
      sample = [
    ...
    counter = 1
    start_time = time.time() #時間從0開始
    
    could_load, checkpoint_counter = self.load(self.checkpoint_dir, config) #load checkpoint
    if could_load:
    ...
    for epoch in xrange(config.epoch):
      ...
      shuffle(self.data) #打亂數據
      batch_idxs = min(len(self.data), config.train_size) // config.batch_size #一個epochs的次數

      for idx in xrange(0, batch_idxs):
        self.experience_flag = not bool(idx % 2)

        batch_files = self.data[idx*config.batch_size:(idx+1)*config.batch_size] #取data 數量
        batch = [
              get_image(batch_file,     #data預處理到[-1,1]
                        input_height=self.input_height,
                        input_width=self.input_width,
                        resize_height=self.output_height,
                        resize_width=self.output_width,
        ...
        if self.grayscale:  
            ...
            try:
                ...
                pdb.set_trace()
        batch_labels = self.get_y(batch_files) #取label
        #噪聲Z 高斯函數(image)
	...
	# TODO
        #print(batch_z.shape)
        np.save('batch_z.npy', batch_z)  #儲存Z

        if self.can:  #CAN模型 tensorboard graph執行
        #update D  feed_dict是餵資料的意思
        ...
        #Update G: don't need labels or inputs
         ...
         #執行Acc
          ...
          # Update D network
          _, summary_str = self.sess.run([self.d_optim, self.clip_D, self.d_sum, self.d_loss],
            feed_dict={
              self.inputs: batch_images,
              self.z: batch_z,
              self.y:batch_labels,
          ...
          # Update G network
          _, summary_str = self.sess.run([self.g_optim, self.g_sum, self.g_loss],
            feed_dict={
              self.z: batch_z,
              self.y: batch_labels,
            })
        ...
        counter += 1
        #CAN模型 輸出型式 Epoch:  time: D-loss: G-loss:   Acc:
        if self.can:
        ...
        #執行次數第30次 執行一次SAMPLE 並儲存一張圖和打印值
        if np.mod(counter, 100) == 1:
          if config.dataset == 'wikiart':
            ...
            print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss)) #打印
  ...
  def test(self, config):
    ...
    if self.y_dim:
      ...
      if (self.grayscale):
        ...
        sample_inputs = np.array(sample).astype(np.float32)
      sample_labels = self.get_y(sample_files) #sample label
    ...
    # batch_z
    batch_z = np.load('batch_z.npy') #讀取
    batch_z_result = self.sess.run([self.G], #執行
            feed_dict={  #餵值
              self.z: batch_z,
            })[0]
    np.save('batch_z_result.npy', batch_z_result) #存檔

    # adjust_batch_z
    adjust_batch_z = np.load('adjust_batch_z.npy')
    adjust_batch_z_result = self.sess.run([self.G],
            feed_dict={
              self.z: adjust_batch_z,
            })[0]
    np.save('adjust_batch_z_result.npy', adjust_batch_z_result)
#D網絡
    ...
    with tf.variable_scope("discriminator") as scope:
      ...
      if self.can:
        ...
        if old_model:
          ...
          h0 = lrelu(conv2d(image, self.df_dim, k_h=4, k_w=4, name='d_h0_conv',padding='VALID')) #32*4*4
          h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim*2, k_h=4, k_w=4, name='d_h1_conv', padding='VALID')))#64*4*4
        h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim*4, k_h=4, k_w=4, name='d_h2_conv', padding='VALID')))#128*4*4
        h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim*8, k_h=4, k_w=4, name='d_h3_conv', padding='VALID')))#256*4*4
        h4 = lrelu(self.d_bn4(conv2d(h3, self.df_dim*16, k_h=4, k_w=4, name='d_h4_conv', padding='VALID')))#512*4*4
        shape = np.product(h4.get_shape()[1:].as_list())
        h5 = tf.reshape(h4, [-1, shape])
        #linear layer to determine if the image is real/fake  全連接層
        h5_ = tf.nn.dropout(h5, 0.5)
        r_out = linear(h5_, 1, 'd_ro_lin')

        #fully connected layers to classify the image into the different styles.
        h6 = lrelu(linear(h5, 1024, 'd_h6_lin')) #1024
        h7 = lrelu(linear(h6, 512, 'd_h7_lin')) #512
        c_out = linear(h7, self.y_dim, 'd_co_lin') #k label數量
        c_out = tf.nn.dropout(c_out, 0.5)
        c_softmax = tf.nn.softmax(c_out) #算機率

        return tf.nn.sigmoid(r_out), r_out, c_softmax, c_out #D , D_logits , D_c , D_c_logits
      else:
  ...
  def generator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      if self.can:
        ...
        z_ = linear(
            z, self.gf_dim*16*s_h64*s_w64, 'g_h0_lin') #線性化 後變為 1024*4*4

        h0 = tf.reshape(
            z_, [-1, s_h64, s_w64, self.gf_dim * 16 ]) #反卷積[-1,4,4,1024]
        h0 = lrelu(self.g_bn0(h0)) #先batch_norm 再leakyRelu

        h1 = self.upsample(
             h0, [-1, s_h32, s_w32, self.gf_dim*16], name='g_h1') #反卷積[-1,8,8,1024]
        h1 = lrelu(self.g_bn1(h1))  #先batch_norm 再leakyRelu

        h2 = self.upsample(
             h1, [-1, s_h16, s_w16, self.gf_dim*8], name='g_h2') #反卷積[-1,16,16,512]
        h2 = lrelu(self.g_bn2(h2)) #先batch_norm 再leakyRelu

        h3 = self.upsample(
            h2, [-1, s_h8, s_w8, self.gf_dim*4], name='g_h3') #反卷積[-1,32,32,256]
        h3 = lrelu(self.g_bn3(h3)) #先batch_norm 再leakyRelu

        h4 = self.upsample(
            h3, [-1, s_h4, s_w4, self.gf_dim*2], name='g_h4') #反卷積[-1,64,64,128]
        h4 = lrelu(self.g_bn4(h4)) #先batch_norm 再leakyRelu

        h5 = self.upsample(
            h4, [-1, s_h2, s_w2, self.gf_dim], name='g_h5') #反卷積[-1,128,128,64]
        h5 = lrelu(self.g_bn5(h5)) #先batch_norm 再leakyRelu

        h6 = self.upsample(
            h5, [-1, s_h, s_w, self.c_dim], name='g_h6') #反卷積[-1,256,256,3]
        h6 = tf.nn.dropout(h6, 0.5)
        return tf.nn.tanh(h6) #tanh [-1,1]
      else:
        """
...
#存檔
  def save(self, checkpoint_dir, step, config):
    ...
    if not config.use_default_checkpoint:
      checkpoint_dir = os.path.join(checkpoint_dir, self.model_dir)#checkpint 路徑

    if not os.path.exists(checkpoint_dir): #文件是否存在
      os.makedirs(checkpoint_dir) #建文件
...
#讀取checkpoint
  def load(self, checkpoint_dir, config):
    ...
    print(" [*] Reading checkpoints...")
    if not config.use_default_checkpoint:
      checkpoint_dir = os.path.join(checkpoint_dir, self.model_dir) #路徑

    ckpt = tf.train.get_checkpoint_state(checkpoint_dir) #取checkpoint文件
    if ckpt and ckpt.model_checkpoint_path:
      ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
      self.saver.restore(self.sess, os.path.join(checkpoint_dir, ckpt_name)) #載入先前checkpoint
      counter = int(next(re.finditer("(\d+)(?!.*\d)",ckpt_name)).group(0))
      print(" [*] Success to read {}".format(ckpt_name))
      return True, counter
    else:
